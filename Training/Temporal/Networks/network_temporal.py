...
class NetworkHandler():
    ...
    def lstm(self, x, output_size, return_sequences=False, activation="tanh"):
        self.lstm_layers += 1
        x = LSTM(output_size, activation=activation, return_sequences=return_sequences)(x)
        return x
...
def load_network(conf):
    ...
    for measure in input_measures:
        ...
        inputs.append(input_layer)
        input_layer = net.dense(input_layer, 10)
        x = concatenate([x, input_layer])
    #print(x)
 
    #x = net.cudnnlstm(x, 1000, return_sequences=True)
    #x = net.dropout(x, 0.5)
    #x = net.cudnnlstm(x, 500, return_sequences=True)
    x = net.lstm(x, 100, return_sequences=True)
    x = net.lstm(x, 50, return_sequences=True)
    x = net.lstm(x, 10, return_sequences=False)

    #x = net.cudnnlstm(x, 50, return_sequences=True)
    #x = net.cudnnlstm(x, 10, return_sequences=False)
