...
class NetworkHandler():
    ...
    def lstm(self, x, output_size, return_sequences=False):
        self.lstm_layers += 1
        x = LSTM(output_size, return_sequences=return_sequences, activation="tanh")(x)
        return x
...
def load_network(conf):
    ...
    for measure in input_measures:
        ...
        #else:
        input_layer = Input([input_size_data["Sequence_length"]] + input_size_data[measure], name="input_" + measure)
        inputs.append(input_layer)
        input_layer = net.dense(input_layer, 10, activation_function="tanh")
        if first:
            x_measure = input_layer
            first = False
        else:
            print(x_measure)
            print(input_layer)
            x_measure = concatenate([x_measure, input_layer])

    x_measure = net.lstm(x_measure, 256, return_sequences=False)
    x_measure = net.dropout(x_measure, 0.3, td=False)
    x = concatenate([x, x_measure])
    #print(x)
    ...
    x = net.dense(x, 100, td = False, activation_function="tanh")
